...
export function start(apiKeyName, startBase = base)
{
    return getHippieInstance(startBase, apiKeyName);
}
...
export function report(message, extras = '') {
    ...
    console.log(`${logMethod(request.method)} ${logUrl(request.uri.href)} took ${request.elapsedTime}ms ${extras}`);
    return message;
}
