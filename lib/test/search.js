    ...
    */
    await start('hive')
        ...
        .expect((res, body, next) => {
            ...
            if (err) {
                logErrors("Error: Search limit did not match the returned result count");
            }
            next();
        })
    ...
    */
    await start('hive')
        ...
        .expect((res, body, next) => {
            ...
            if (err) {
                logErrors("Error: At least one returned search result didn't belong to the given pool");
            }
            next();
        })
    ...
    */
    await start('hive')
        ...
        .expect((res, body, next) => {
            ...
            if (err) {
                logErrors("Error: Returned results from searching by hash id did not match the expected results");
            }
            next();
        })
        .expectStatus(200)
