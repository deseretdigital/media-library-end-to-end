...
export default async function run() {
  ...
  await start("utah")
    ...
    .expect((res, body, next) => {
      ...
      if (data !== null) {
        logErrors(
          "PUT (UTAH): Expected data to be null. Received: ",
          JSON.stringify(data)
        );
        next(true);
      } else {
  ...
  await start("utah")
    ...
    .expect((res, body, next) => {
      ...
      if (data !== null) {
        logErrors(
          "PUT with ID (UTAH): Expected data to be null. Received: ",
          JSON.stringify(data)
        );
        next(true);
      } else {
  ...
  await start("hive")
    ...
    .expect((res, body, next) => {
      ...
      if (
        JSON.stringify(data) !==
        JSON.stringify(assignIn({ id: hivePoolId }, hiveResult))
      ) {
  ...
  await start("utah")
    ...
    .expect((res, body, next) => {
      ...
      if (
        JSON.stringify(data) !==
        JSON.stringify(assignIn({ id: utahPoolId }, utahResult))
      ) {
  ...
  await start("hive")
    ...
    .expect((res, body, next) => {
      ...
      if (!found) {
        logErrors(
          "GET owned pools (HIVE): Did not find the pool with id: ",
          hivePoolId
        );
      }
      next();
  ...
  await start("hive")
    ...
    .expect((res, body, next) => {
      ...
      if (!found) {
        logErrors(
          `GET all pools (HIVE): Could not find pool in results. id: ${hivePoolId}`
        );
      }
      next();
    })
  ...
  await start("hive")
    ...
    .expect((res, body, next) => {
      ...
      if (id !== hivePoolId) {
        logErrors(
          `GET pool by owned id (HIVE): Did not find pool with id: ${hivePoolId}`
        );
      }
      next();
    })
