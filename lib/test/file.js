...
export default async function run() {
    ...
    */
    await start('hive', null, null, false)
        ...
        .expect((res, body, next) => {
            ...
            imagemagick.identify(`${base}/file/${idHash}?` + signUrl(`crop=${crop}`), (err, features) => {
                if (err) {
                    logErrors(err);
                }
                if (features) {
                    if (features.format !== "GIF") {
                        logErrors(`Error: Format returned when passing in crops was expected to be a GIF, but got a ${features.format}`);
                    }
                    if (features["channel statistics"]["pixels"] >= originalPixels) {
                        logErrors(`Error: Pixels returned was expected to be less than ${originalPixels}, but got ${features["channel statistics"]["pixels"]}`);
                    }
                } else {
    ...
    */
    await start('hive', null, null, false)
        ...
        .expect((res, body, next) => {
            ...
            imagemagick.identify(`${base}/file/${idHash}?` + signUrl(`resize=${resize}`), (err, features) => {
                if (err) {
                    logErrors(err);
                }
                if (features) {
                    ...
                    if (features.height !== 25) {
                        logErrors(`Error: The height returned did not match the height we passed in`);
                    }
                } else {
                    logErrors("Sorry, the operation timed out");
                }
            ...
            imagemagick.identify(`${base}/file/${idHash}?` + signUrl(`resize=${resize}`), (err, features) => {
                ...
                if (features) {
                    ...
                    if (features.width !== 25) {
                        logErrors(`Error: The width returned did not match the width we passed in`);
                    }
                } else {
                    logErrors("Sorry, the operation timed out");
                }
        ...
        .expect((res, body, next) => {
            ...
            imagemagick.identify(`${base}/file/${idHash}?` + signUrl(`resize=${resize}`), (err, features) => {
                ...
                if (features) {
                    ...
                    if (features.width !== 25) {
                        logErrors(`Error: The width returned was constrained with constrain set to false`);
                    }
                    if (features.height !== 20) {
                        logErrors(`Error: The height returned was constrained with constrain set to false`);
                    }
                } else {
                    logErrors("Sorry, the operation timed out");
                }
    ...
    */
    await start('hive', null, null, false)
        ...
        .expect((res, body, next) => {
            ...
            imagemagick.identify(`${base}/file/${idHash}?` + signUrl(`overlay=${overlay}`), (err, features) => {
                if (err) {
                    logErrors(err);
                }
            });
    ...
    */
    await start('hive', null, null, false)
        .get(`/file/${idHash}`)
        .expect((res, body, next) => {
            imagemagick.identify(`${base}/file/${idHash}?` + signUrl("grayscale=true"), (err, features) => {
                if (err) {
                    logErrors(err);
                }
                if (features) {
                    const channelStatistics = features['channel statistics'];
                    if (!channelStatistics['gray'] || channelStatistics['red'] || channelStatistics['green'] || channelStatistics['blue']) {
                        logErrors("Error: Grayscale returned something other than a grayscale image");
                    }
                } else {
                    logErrors("Sorry, the operation timed out");
                }
    ...
    */
    await start('hive', null, null, false)
        ...
        .expect((res, body, next) => {
            const quality = 1;
            imagemagick.identify(`${base}/file/${idHash}?` + signUrl(`quality=${quality}`), (err, features) => {
                if (err) {
                    logErrors(err);
                }
            });
    ...
    */
    await start('hive', null, null, false)
        ...
        .expect((res, body, next) => {
            ...
            imagemagick.identify(`${base}/file/${idHash}?` + signUrl(`interlace=${interlace}`), (err, features) => {
                if (err) {
                    logErrors(err);
                }
            });
    ...
    */
    await start('hive', null, null, false)
        ...
        .expect((res, body, next) => {
            ...
            imagemagick.identify(`${base}/file/${idHash}?` + signUrl(`order=${order}&crop=${crop}&resize=${resize}`), (err, features) => {
                if (err) {
                    logErrors(err);
                }
                if (features) {
    ...
    */
    await start('hive', null, null, false)
        ...
        .expect((res, body, next) => {
            ...
            imagemagick.identify(`${base}/file/${idHash}?filter=${filter}&c=${config.clientId}`, (err, features) => {
                if (err) {
                    logErrors(err);
                }
            });
    ...
    */
    await start('hive', null, null, false)
        ...
        .expect((res, body, next) => {
            ...
            imagemagick.identify(`${base}/file/${idHash}?filter=${filter}&` + signUrl(`resize=${resize}`), (err, features) => {
                if (err) {
                    logErrors(err);
                }
                if (features) {
            ...
            if (body) {
                ...
                if (body.data) {
                    if (body.data.message) {
                        if (body.data.message.indexOf("Successfully requested renditions") === -1) {
                            logErrors("Error: Post action did not return success message instead returned " + body.data.message)
                        }
                    } else {
                        logErrors("Error: Post action no body message returned when expected");
                    }
                } else {
