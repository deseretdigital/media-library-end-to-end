...
export default async function run() {
    ...
    //TODO test permission errors when you post a dupe and don't own it
    //TODO test uploading - unknown method
    //TODO delete
    ...
    **/
    ...
    const data = {
        contents: Buffer.from(fs.readFileSync(__dirname + '/../img/reef.jpeg')).toString('base64')
    };
...
function assertValidMLResponse(body) {
    const assertion = !!(body.error === null
        && body.data
        && body.data.id
        && body.data.id_hash
        && body.data.name
        ...
        && body.data.file_hash);
    if(!assertion) {
        logErrors('Response not formatted correctly. The following fields are required: \n\
